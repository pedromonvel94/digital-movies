...
const director = new mongoose.Schema(
    {
        nombre: { type: String, required: [true, "El nombre es obligatorio"], trim: true }, //Le indico a mongoose que quiero que el documento tenga el par clave-valor: nombre:valor y que ese valor cumpla con los requisitos que le coloque, osea que sea String, que sea required, que se le haga trim para eliminar espacios, y unique.
...
export const Director = mongoose.model("Director", director); //Esta linea sirve para convertir el molde que creamos arriba en una herramienta la cual podamos usar, basicamente ("Crea un Model llamado Director a partir del schema, y lo registra dentro de Mongoose")